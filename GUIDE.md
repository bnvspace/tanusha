...
Данные пользователей хранятся в MariaDB/MySQL, а загруженные файлы — в `uploads/`. Git эти данные не хранит, поэтому перед деплоем, переносом или ручными правками нужно снять резервную копию:

```bash
php scripts/backup.php
```

Архив появится в `storage/backups/` и будет содержать `database.sql`, `manifest.txt` и копию `uploads/`.
